"""Session Knowledge Graph implementation for DeepResearch-Lite.
...
class SessionKnowledgeGraph:
    ...
    def find_conflict(self, fact: FactAssertion) -> Optional[FactAssertion]:
        ...
        For example:
        Existing: (Solid State Battery, cost, $140/kWh)
        Incoming: (Solid State Battery, cost, $110/kWh)
        """
        ...
        for existing in self._sp_index[sp_key]:
            # If object values differ and either is numeric or explicitly distinct statements
    ...
    def to_mermaid(self) -> str:
        ...
        lines = ["graph TD"]
        for i, fact in enumerate(self.facts[:30]):  # Limit top 30 for visualization
